...
import {
  ...
  getRadarScores, getCompositeScore, getHub,
} from '../lib/process';
import { WEEKLY_CAPACITY } from '../lib/config';

// ── Palette ──────────────────────────────────────────────
const C = {
  bg: '#0a0a0a', card: '#111', cardBorder: '#1c1c1c', hover: '#181818',
  teal: '#1D9E75', blue: '#378ADD', red: '#E24B4A', purple: '#7F77DD',
  amber: '#EF9F27', pink: '#D4537E', gray: '#555',
  tealDim: '#0F6E56', blueDim: '#185FA5', redDim: '#A32D2D',
  text: '#d4d4d4', muted: '#737373', faint: '#404040',
};

const URGENCY_COLORS = { Overdue: C.red, 'Due soon': C.amber, Upcoming: C.blue, 'No date': C.gray };
const PIE_STATUS = { Completed: C.teal, 'In progress': C.blue, Overdue: C.red };
...
function Badge({ status }) {
  const s = {
    Completed: 'bg-emerald-950/50 text-emerald-400 border-emerald-800/30',
    'In progress': 'bg-blue-950/50 text-blue-400 border-blue-800/30',
    Overdue: 'bg-red-950/50 text-red-400 border-red-800/30',
  };
  return (
    <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-medium border ${s[status] || 'bg-neutral-800 text-neutral-400 border-neutral-700'}`}>
      {status}
    </span>
  );
}
...
function HBar({ name, value, max, color = C.purple, suffix = '' }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span className="text-xs text-neutral-500 w-24 text-right truncate">{name}</span>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-neutral-300">
      ...
      <header className="border-b border-[#1c1c1c] px-4 py-3 sticky top-0 z-30 bg-[#0a0a0a]/95 backdrop-blur">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] text-neutral-600">Project Hub — V3</span>
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-5 space-y-4">
        ...
        {tab === 6 && <TabProjects projects={data.projects} funnel={data.funnel} tasks={filteredTasks} />}
        {tab === 7 && <TabTimeline d={processed} />}
        {tab === 8 && <TabRadar d={processed} />}

        <p className="text-[11px] text-neutral-700 text-center pt-2 pb-4">
          Last synced {new Date(data.fetchedAt).toLocaleString()} · Refreshes every 2 days
        </p>
      </main>
    </div>
  );
...
function TabOverview({ d, tasks }) {
  ...
  const maxTasks = members.length > 0 ? members[0].total : 1;

  const statusPie = {};
  tasks.forEach(t => { statusPie[t.status] = (statusPie[t.status] || 0) + 1; });
  const pieData = Object.entries(statusPie).map(([name, value]) => ({ name, value }));
        ...
        <Card title="Task load by member">
          <div className="space-y-2">
            {members.filter(m => m.name !== 'Unassigned').slice(0, 10).map(m => {
              const max = m.overdue + m.dueSoon + m.upcoming + m.noDate;
              return (
    ...
    <>
      ...
      <Card title="Weekly output — Dubai vs Lebanon">
        ...
        <div className="flex gap-4 mt-2 text-[10px] text-neutral-600">
          <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: C.blue }} />Dubai</span>
          <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: C.amber }} />Lebanon</span>
        </div>
      </Card>
...
// ══════════════════════════════════════════════════════════
// TAB 6: PROJECTS & FUNNEL
// ══════════════════════════════════════════════════════════
function TabProjects({ projects, funnel, tasks }) {
  ...
  return (
    <>
      ...
      <Card title="Task volume by project">
        <ResponsiveContainer width="100%" height={Math.max(160, projects.length * 40)}>
          <BarChart data={projects} layout="vertical">
            <CartesianGrid strokeDasharray="3 3" stroke="#1c1c1c" />
            <XAxis type="number" tick={{ fill: '#555', fontSize: 11 }} />
            <YAxis dataKey="name" type="category" width={120} tick={{ fill: '#555', fontSize: 11 }} />
            <Tooltip {...TT_STYLE} />
            <Bar dataKey="completedTasks" stackId="a" fill={C.teal} name="Completed" />
            <Bar dataKey="overdueTasks" stackId="a" fill={C.red} name="Overdue" />
            <Bar dataKey="remainingTasks" stackId="a" fill={C.gray} name="Remaining" radius={[0, 3, 3, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </Card>
    </>
...
// ══════════════════════════════════════════════════════════
// TAB 7: TIMELINE & HEATMAP
// ══════════════════════════════════════════════════════════
function TabTimeline({ d }) {
  const { heatmap, gantt } = d;
  const maxCount = Math.max(...heatmap.map(h => h.count), 1);

  // Group heatmap by weeks (rows of 7)
  const weeks = [];
  ...
  return (
    <>
      <Card title="Workload heatmap — tasks due per day">
        <p className="text-[11px] text-neutral-600 mb-3">Darker = more tasks due that day</p>
        ...
        <div className="flex gap-4 mt-3 text-[10px] text-neutral-600">
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded" style={{ background: '#141414' }} />Low</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded" style={{ background: C.teal }} />High</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded" style={{ background: '#181818' }} />Past</span>
        </div>
      </Card>

      <Card title="Gantt-style task timeline">
        <p className="text-[11px] text-neutral-600 mb-3">Tasks plotted by start → due date</p>
  ...
  return (
    <>
      ...
      <Card title="Full performance scorecard">
        <p className="text-[11px] text-neutral-600 mb-3">5-dimension composite score — higher is better</p>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-[#1c1c1c]">
