...
export async function fetchAllData() {
  ...
  for (const gid of projectGids) {
    ...
    // Fetch subtasks for any top-level task that has them
    ...
    // Flatten and attach parent section info to subtasks
    const allSubtasks = [];
    tasksWithSubtasks.forEach((parent, i) => {
      ...
      subtasksArrays[i].forEach(sub => {
        // Inherit section from parent if subtask doesn't have one
        if (!sub.memberships?.length) {
          sub._inheritedSection = parentSection;
        }
