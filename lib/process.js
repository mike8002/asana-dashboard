import { HUB_MAP, HUB_BY_EMAIL, DEFAULT_HUB, WEEKLY_CAPACITY, TREND_WEEKS } from './config';

export function getHub(assignee, email) {
  // Prefer email match (unique identifier, matches AD exactly)
  if (email) {
    const byEmail = HUB_BY_EMAIL[email.toLowerCase()];
    if (byEmail) return byEmail;
  }
  // Fall back to name match for edge cases
  if (assignee && HUB_MAP[assignee]) {
    return HUB_MAP[assignee];
  }
  return DEFAULT_HUB;
}
...
export function getMemberStats(tasks) {
  ...
  tasks.forEach(t => {
    ...
    if (!map[t.assignee]) map[t.assignee] = {
      name: t.assignee, email: t.email || '', hub: getHub(t.assignee, t.email),
