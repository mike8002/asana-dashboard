...
export function getWeeklyTrend(tasks) {
  const weeks = getLastNWeeks(TREND_WEEKS);
  return weeks.map(w => ({
...
export function getVelocityData(members) {
...
// ── NEW: breakdown helpers ────────────────────────────────

function getBreakdown(tasks, field) {
  const map = {};
  tasks.forEach(t => {
    if (!t[field]) return;
    // Custom fields with multiple values come back comma-separated
