...
// ═══════════════════════════════════════════════════════════
// DYNAMIC WEEK WINDOW
// Finds the most recent week with any task activity, then shows
// the N weeks ending at that point. This way the chart auto-
// adjusts to whatever period your data actually lives in.
// ═══════════════════════════════════════════════════════════
function getActivityWeeks(tasks, count = TREND_WEEKS) {
  // Collect all weeks where anything happened
  ...
  if (activeWeeks.size === 0) {
    // No activity at all — fall back to last N weeks from today
    return getLastNWeeks(count);
  }

  // Find the most recent active week (or today, whichever is later)
  ...
  // Build N weeks ending at the anchor
...
export function getVelocityData(members, tasks) {
  // Use 7-week window based on actual activity
